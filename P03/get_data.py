...
def write_country_data(file, data):
    ...
    with open(file, 'a') as f:
        ...
        for country in countries.itertuples(index=False):
            f.write(
                f"INSERT INTO countries VALUES ({as_str(country.code)}, {as_str(country.name)}, {as_str(country.region)});\n")
...
def write_team_data(file, data):
    ...
    with open(file, 'a') as f:
        ...
        for team in teams.itertuples(index=False):
            f.write(
                f"INSERT INTO teams VALUES ({as_str(team.team)}, {as_str(team.team_country_code)});\n")

        f.write("\n\n")


def write_rider_data(file, data):
    riders = data[['bib', 'rider', 'dob', 'team']
                  ].drop_duplicates().reset_index(drop=True)
    with open(file, 'a') as f:
        f.write("-- Insert riders Data\n")
        for rider in riders.itertuples(index=False):
            f.write(
                f"INSERT INTO riders VALUES ({rider.bib}, {as_str(rider.rider)}, {as_str(rider.dob)}, {as_str(rider.team)});\n")
...
def write_rider_from_data(file, data):
    ...
    with open(file, 'a') as f:
        f.write("-- Insert riders_from Data\n")
        for rf in rider_from.itertuples(index=False):
            f.write(
                f"INSERT INTO riders_from VALUES ({rf.bib}, {as_str(rf.rider_country_code)});\n")
...
def write_location_data(file, data):
    ...
    with open(file, 'a') as f:
        ...
        for location in locations.itertuples(index=False):
            f.write(
                f"INSERT INTO locations VALUES ({as_str(location.location)}, {as_str(location.country)});\n")
...
def write_stage_data(file, data):
    ...
    with open(file, 'a') as f:
        ...
        for stage in stages.itertuples(index=False):
            f.write(
                f"INSERT INTO stages VALUES ({stage.stage}, {as_str(stage.day)}, {as_str(stage.start_location)}, {as_str(stage.finish_location)}, {stage.length}, {as_str(stage.type)});\n")
...
def write_riders_exits_data(file, data_exit, data):
    ...
    with open(file, 'a') as f:
        f.write("-- Insert riders_exits Data\n")
        for exit in processed.itertuples(index=False):
            f.write(
                f"INSERT INTO riders_exits VALUES ({exit.bib}, {exit.stage}, {as_str(exit.reason)});\n")
        f.write("\n\n")
